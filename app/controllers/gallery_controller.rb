class GalleryController < ApplicationController
  def index
    @cloth_gallery = Store.all
  end

  def search
    word = '%' + params[:keyword] + '%'
    @search_results = Store.find_by_sql ["Select * from stores WHERE name like ? or brand like ? or size like ? or description like ?",word,word,word,word]   
  end
